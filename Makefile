...
UTILS_TARGET = $(wildcard src/utils/*.cpp)
...
ifdef PLATFORM
ifeq ($(PLATFORM),WIN64)
	$(CXX) $(TARGETS) $(UTILS_TARGET) $(GAME_TARGETS) $(SCENE_TARGETS) $(GAME_COMPONENTS_TARGETS) $(SCENE_ENTITIES_TARGETS) $(MENU_TARGETS) $(MENU_COMPONENTS_TARGETS) $(INCLUDES) \
	$(CXXFLAGS) \
