...
menu_background::menu_background(){
    ...
    tile_width = data::textures::textures[BACKGROUND_TILE].width;
    tile_height = data::textures::textures[BACKGROUND_TILE].height;
}
...
void menu_background::render(){
    ...
    for (int y = 0; y < ceil(screen_height/tile_height) + 1; y++){
        for (int x = 0; x < ceil(screen_width/2.5/tile_width); x++){
            gfx::enable_texture(data::textures::textures[BACKGROUND_TILE]);
